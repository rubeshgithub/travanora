...
export function PhoneInput({ value, onChange, label, error }) {
    ...
    function handleNum(raw) {
        const digits = raw.replace(/\D/g, '');
        setNum(digits);
        onChange(code + digits);
    }
    return (_jsxs("div", { children: [_jsx("label", { className: "label-text", children: label }), _jsxs("div", { className: "flex gap-2 mt-1", children: [_jsx("select", { value: code, onChange: (e) => handleCode(e.target.value), className: "input-field w-28 flex-shrink-0", children: COUNTRY_CODES.map((c) => (_jsxs("option", { value: c.code, children: [c.flag, " ", c.code] }, `${c.code}-${c.label}`))) }), _jsx("input", { type: "tel", value: num, onChange: (e) => handleNum(e.target.value), inputMode: "numeric", placeholder: "99887766", className: `input-field flex-1 ${error ? 'input-field-error' : ''}` })] }), error && _jsx("p", { className: "text-[13px] text-red-500 mt-1", children: error })] }));
}
